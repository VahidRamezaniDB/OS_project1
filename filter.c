...
int compute_gauss_avg(int im[IM_SIZE+KERNEL_SIZE-1][IM_SIZE+KERNEL_SIZE-1],int kernel[KERNEL_SIZE][KERNEL_SIZE],int i, int j){
    double res=0.0;
...
void apply_kernel(int im[IM_SIZE+KERNEL_SIZE-1][IM_SIZE+KERNEL_SIZE-1],int output_im[IM_SIZE][IM_SIZE],int kernel[KERNEL_SIZE][KERNEL_SIZE]){
    ...
    for (int i=0;i<IM_SIZE;i++){
        for (int j=0;j<IM_SIZE;j++){
            output_im[i][j]=compute_gauss_avg(im,kernel,i,j);
        }
    }
